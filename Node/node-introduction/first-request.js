//http => hyper text transfare protocol
    ...
    if(req.method == "POST" && req.url == "/store"){
        ...
        req.on("end",() => {
            ...
            if(object.name != undefined && object.email != undefined){
                ...
                res.write(JSON.stringify({status:true,message:"Success",data:users}));
                
            }else{
                res.writeHead(400,
                    {'Content-Type':'application/json'
                });
                res.write(JSON.stringify({status:false,message:"Enter required data"}));
            }
            res.end();
        });
        }
        ...
        res.write(JSON.stringify(
            {status:true, message:"Success", data:users,users:users, 
        }));
        res.end();
        }
    else{
        ...
        res.write(JSON.stringify({status:false , message:'requested is not defined '}))
        res.end();
    }
